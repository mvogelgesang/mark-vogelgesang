...
description: "How does someone who is not a visual designer take on the task of creatign a design system?"
hero:
  src: ""
  alt: ""
---
...
Over the holidays, I spent some time to switch the site from Gatsby to Astro and
add a dark mode toggle (see <i class="fas fa-adjust"></i> above). The switch
from Gatsby to Astro was fairly straight forward and resulted in a codebase that
will be easier to maintain. In the process of implementing the dark mode toggle,
I found inconsistencies with my original SASS which made the conversion harder
than it had to be. In response, I wanted to create something that was easier to
extend while also looking less thrown together. This post will cover the visual
design choices for the site including an 8pt grid, color swatches, and site
typography. I hesitate to call it a "design-system" but these are some baby
...
design system documentation. I found Atlassian and IBM's documentation to
explain the theory and practice of implementing a design system in a way that is
consumable by a wide audience. The
[Foundations](https://atlassian.design/foundations/) section of Atlassian's
website was a source I went back to again and again.

The more I read about design systems, the more I saw mentiones of using an 8pt
grid system which was a new concept to me. Both IBM and Atlassian utilize 8pt
grids as cornerstones to their designs. In addition to reading their content,
Vitsky discusses the topic and provides examples in Figma. Both links are
member-only Medium posts.
...
Following many of the principles of the 8pt grid, I established a typography
leveraging Montserrat in various weights and sizes.

![mvogelgesang.com typography on a light background](/posts/20221230/Light.png)
![mvogelgesang.com typography on a dark background](/posts/20221230/Dark.png)

| Font            | Variable     | Element/ Purpose | Font Size (px) | Line Height (px) |
| --------------- | ------------ | ---------------- | -------------- | ---------------- |
| Montserrat      | SemiBold 600 | h1               | 40             | 48               |
| Montserrat      | SemiBold 600 | h2               | 32             | 40               |
| Montserrat      | SemiBold 600 | h3               | 24             | 28               |
| Montserrat      | SemiBold 600 | h4               | 16             | 24               |
| Montserrat      | Regular 400  | p / prose        | 16             | 24               |
| Montserrat      | Regular 400  | prose small      | 12             | 24               |
| Source Code Pro | Regular 400  | pre, code        | 12             | 20               |
...
As it stands, the switch to color swatches and typography settings are  incomplete and the SASS files are a mix between the old and new. A subsequent update using design tokens should make hte switch complete. In subsequent updates, I hope to address the following:
